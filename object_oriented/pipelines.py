...
class DatabasePipeline:
    ...
    def __init__(
        self,
        connection_class,
        functions, funcs_types, funcs_config_args, funcs_pool_sizes,
        n_repeats = 50):
        ...
        try:
            ...
            assert isinstance(funcs_types, tuple)
        ...
        for i in range(self.N):
            ...
            try:
                assert funcs_types[i] in self.allowable_types
            except:
            ...
            if i == 0:
                try:
                    assert 'producer' in funcs_types[i]
                except:
                    raise AssertionError('first function must be a producer.')
            if 'producer' in funcs_types[i]:
                try:
            ...
            try:
                assert 'consumer' in funcs_types[i]
            except:
